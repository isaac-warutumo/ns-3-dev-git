...
static inline SequenceNumber32
SEQ64TO32 (SequenceNumber64 seq)
{
  return SequenceNumber32 (seq.GetValue ());
}

static inline SequenceNumber64
SEQ32TO64 (SequenceNumber32 seq)
{
  return SequenceNumber64 (seq.GetValue ());
}

//! wrapper function
static inline MpTcpMapping
...
MpTcpSocketBase::GetTypeId (void)
{
  static TypeId tid =
      TypeId ("ns3::MpTcpSocketBase")
          ...
          .AddAttribute (
              "SocketType", "Socket type of TCP objects.", TypeIdValue (MpTcpSubflow::GetTypeId ()),
              MakeTypeIdAccessor (&MpTcpSocketBase::m_subflowTypeId), MakeTypeIdChecker ())
          .AddAttribute ("Scheduler", "How to generate the mappings",
                         TypeIdValue (MpTcpScheduler::GetTypeId ()),
                         MakeTypeIdAccessor (&MpTcpSocketBase::m_schedulerTypeId),
                         MakeTypeIdChecker ())
          .AddAttribute ("PathManagerMode", "Mechanism for establishing new sub-flows",
...
static const std::string containerNames[MpTcpSocketBase::Maximum] = {"Established", "Others",
                                                                     "Closing"};
...
{
  NS_ASSERT_MSG (InetSocketAddress::IsMatchingType (local) &&
                     InetSocketAddress::IsMatchingType (remote),
                 "only support ipv4");

  NS_LOG_LOGIC ("Trying to add a new subflow "
...
MpTcpSocketBase::OnSubflowClosed (Ptr<MpTcpSubflow> subflow, bool reset)
{
  ...
  SubflowList::iterator it =
      std::remove (m_subflows[Closing].begin (), m_subflows[Closing].end (), subflow);
}
...
static void
onSubflowNewState (Ptr<MpTcpSocketBase> meta, Ptr<MpTcpSubflow> sf, TcpSocket::TcpStates_t oldState,
                   TcpSocket::TcpStates_t newState)
{
  NS_LOG_UNCOND ("onSubflowNewState wrapper");
  meta->OnSubflowNewState ("context", sf, oldState, newState);
}

/* use it to Notify user of, we need a MakeBoundCallback */
void
MpTcpSocketBase::OnSubflowNewState (std::string context, Ptr<MpTcpSubflow> sf,
                                    TcpSocket::TcpStates_t oldState,
                                    TcpSocket::TcpStates_t newState)
{
  NS_LOG_LOGIC ("subflow " << sf << " state changed from " << TcpStateName[oldState] << " to "
                           << TcpStateName[newState]);
...
MpTcpSocketBase::AddSubflow (Ptr<MpTcpSubflow> sflow)
{
  ...
  bool ok;
  ok = sf->TraceConnect ("CongestionWindow", "CongestionWindow",
                         MakeCallback (&MpTcpSocketBase::OnSubflowNewCwnd, this));
  ...
  sf->SetAcceptCallback (MakeNullCallback<bool, Ptr<Socket>, const Address &> (),
...
void
MpTcpSocketBase::ForwardUp (Ptr<Packet> packet, Ipv4Header header, uint16_t port,
...
Ipv4EndPoint *
MpTcpSocketBase::NewSubflowRequest (Ptr<Packet> p, const TcpHeader &tcpHeader,
                                    ...
                                    Ptr<const TcpOptionMpTcpJoin> join)
{
  ...
  NS_ASSERT_MSG (InetSocketAddress::IsMatchingType (fromAddress) &&
                     InetSocketAddress::IsMatchingType (toAddress),
...
MpTcpSocketBase::CreateSingleSubflow (uint16_t randomSourcePort, uint16_t randomDestinationPort)
{
  ...
  NS_LOG_INFO ("Creating Subflow with: sAddr " << sAddr << " sPort " << sPort << "dAddr " << dAddr
                                               << " dPort " << dPort);
  ...
  if (!result)
    {
      NS_LOG_INFO (
          "InitiateSinglesubflow/ Can't add socket: already registered ? Can be because of mptcp");
    }
      ...
      for (auto it = RemoteAddressInfo.begin (); it != RemoteAddressInfo.end (); it++)
        {
          ...
          if (localPort == sPort)
            {
              NS_LOG_UNCOND ("Generated random port is the same as meta subflow's local port, "
                             "increment by +1");
              localPort++;
            }
          if (remotePort == dPort)
            {
              NS_LOG_UNCOND (
                  "Generated random port is the same as meta subflow's peer port, increment by +1");
              remotePort++;
            }
          ...
          subflow->m_synCount = m_synRetries;
          subflow->m_dataRetrCount = m_dataRetries;
          subflow->m_endPoint = m_tcp->Allocate (GetBoundNetDevice (), localAddress, localPort,
          ...
          if (!result)
            {
              NS_LOG_INFO ("InitiateSinglesubflow/ Can't add socket: already registered ? Can be "
                           "because of mptcp");
            }
          subflow->SendEmptyPacket (TcpHeader::SYN);
        }
...
MpTcpSocketBase::PeerClose (SequenceNumber32 dsn, Ptr<MpTcpSubflow> sf)
{
  ...
  if (dsn < m_rxBuffer->NextRxSequence () || m_rxBuffer->MaxRxSequence () < dsn)
    {
      NS_LOG_INFO ("dsn " << dsn << " out of expected range [ " << m_rxBuffer->NextRxSequence ()
...
MpTcpSocketBase::DoRetransmit ()
{
  ...
  if (m_state == SYN_SENT)
    {
      if (m_synCount > 0)
        {
          NS_FATAL_ERROR ("TODO, first syn didn't reach it should be resent. Maybe this shoudl be "
                          "let to the subflow");
        }
      else
        {
  ...
  DumpRxBuffers (0);
  NS_FATAL_ERROR ("TODO later, but for the tests only, it should not be necesssary ?! Check for "
                  "anything suspicious");
}
  ...
  for (uint32_t i = 1; i < ipv4->GetNInterfaces (); i++)
    {
      ...
      uint8_t addrId = j;
      std::cout << " advertising " << interfaceAddr.GetLocal () << " "
                << m_endPoint->GetLocalPort () << std::endl;
  ...
  for (uint32_t i = 0; i < ipv4->GetNInterfaces (); i++)
    {
      ...
      if (interfaceAddr.GetLocal () == Ipv4Address::GetLoopback ())
        continue;
      NS_LOG_INFO (" Adding local addresses " << interfaceAddr.GetLocal () << " "
                                              << m_endPoint->GetLocalPort ());
      LocalAddressInfo.push_back (
          std::make_pair (interfaceAddr.GetLocal (), m_endPoint->GetLocalPort ()));
    }
}
...
void
MpTcpSocketBase::MoveSubflow (Ptr<MpTcpSubflow> subflow, mptcp_container_t from,
                              mptcp_container_t to)
{
  NS_ASSERT (from != to);
  SubflowList::iterator it =
      std::find (m_subflows[from].begin (), m_subflows[from].end (), subflow);

  if (it == m_subflows[from].end ())
    {
      NS_LOG_ERROR ("Could not find subflow in *from* container. It may have already been moved by "
                    "another callback ");
      DumpSubflows ();
      return;
    }
  ...
  for (int i = 0; i < Maximum; ++i)
    {
      ...
      for (SubflowList::const_iterator it = m_subflows[i].begin (); it != m_subflows[i].end ();
           it++)
        {
  ...
  if (subflow->IsMaster ())
    {
      ...
      if (m_state == SYN_SENT || m_state == SYN_RCVD)
        ...
        {
          NS_FATAL_ERROR ("Unhandled case where subflow got established while meta in "
                          << TcpStateName[m_state]);
        }
      InetSocketAddress addr (subflow->m_endPoint->GetPeerAddress (),
                              subflow->m_endPoint->GetPeerPort ());
...
MpTcpSocketBase::OnSubflowEstablishment (Ptr<MpTcpSubflow> subflow)
{
  ...
  if (subflow->IsMaster ())
    {
      NS_LOG_LOGIC ("Master subflow established, moving meta from " << TcpStateName[m_state]
      ...
      if (m_state == SYN_SENT || m_state == SYN_RCVD)
        ...
        {
          NS_LOG_WARN ("Unhandled case where subflow got established while meta in "
                       << TcpStateName[m_state]);
        }
...
void
MpTcpSocketBase::SetSubflowConnectCallback (Callback<void, Ptr<MpTcpSubflow>> connectionSucceeded,
  ...
  for (int i = 0; i < Closing; ++i)
    {
      ...
      m_subflows[Closing].insert (m_subflows[Closing].end (), m_subflows[i].begin (),
                                  m_subflows[i].end ());
      m_subflows[i].clear ();
    }
}
...
void
MpTcpSocketBase::ReceivedAck (SequenceNumber32 dack, Ptr<MpTcpSubflow> sf, bool count_dupacks)
{
...
MpTcpSocketBase::TimeWait ()
{
  ...
  CancelAllTimers ();
  m_timewaitEvent =
      Simulator::Schedule (timewait_duration, &MpTcpSocketBase::OnTimeWaitTimeOut, this);
}
...
MpTcpSocketBase::OnTimeWaitTimeOut (void)
{
  ...
  NS_LOG_UNCOND ("after timewait timeout, there are still " << m_subflows[Closing].size ()
                                                            << " subflows pending");
